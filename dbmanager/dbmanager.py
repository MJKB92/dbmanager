...
class DBManager:
    ...
    def insert_to_col(self, d_data, col_name, db_name, correct_encode=False,
                      remove_none=False):
        """
        inserting or writing document to a collection of database with the option of imposing custom encoder to
    ...
    def upsert_one(self, d_data, col_name, db_name, d_filter, correct_encode=False, remove_none=False):
        """
        inserting or updating document based on a filter to a collection of database with the option of imposing custom
        encoder to have a proper format for storing data in mongodb
        ...
        col = self.client[db_name][col_name]
        data = self._encode_data_for_insert(d_data, correct_encode, remove_none)
        col.update_one(d_filter, {'$set': data}, upsert=True)
